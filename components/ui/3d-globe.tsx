...
interface Globe3DProps {
  ...
  /** Callback when the globe has rendered its first frame */
  onReady?: () => void;
}
...
function RotatingGlobe({
   ...
   *
   * useTexture has resolved at this point, so the
   * Earth assets are loaded.
   *
   * useFrame then runs only when React Three Fiber
   * has an actual render loop. We call onReady once,
   * meaning the page is released only after the globe
   * has actually entered the WebGL render lifecycle.
   */
  useFrame(() => {
    if (readyRef.current) {
      return;
