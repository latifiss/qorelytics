  ...
  const inputContent = (
    <div className="relative">
      ...
      <div className={cn('w-full rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-tab relative z-20', 'bg-white dark:bg-[#22282b]', className)}>
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-neutral-200 dark:border-neutral-800"><div className="flex items-center gap-2 min-w-0"><button type="button" onClick={() => setIsModalOpen(true)} disabled={disabled} className="p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0" aria-label="Attach file"><div className="block dark:hidden"><ScanIcon size={24} color="#525252" /></div><div className="hidden dark:block"><ScanIcon size={24} color="#FFFFFF" /></div></button><div className="relative min-w-0 w-[calc(100vw-160px)] max-w-[180px] md:w-auto md:max-w-none md:overflow-visible"><div className="flex items-center gap-1 overflow-x-auto whitespace-nowrap scrollbar-none [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><ModeButton mode="investigate" isSelected={selectedMode === 'investigate'} onClick={() => handleModeChange('investigate')} className="h-8 px-3 text-xs shrink-0" /><ModeButton mode="strategy" isSelected={selectedMode === 'strategy'} onClick={() => handleModeChange('strategy')} className="h-8 px-3 text-xs shrink-0" /><ModeButton mode="analyze" isSelected={selectedMode === 'analyze'} onClick={() => handleModeChange('analyze')} className="h-8 px-3 text-xs shrink-0" /></div><div className="pointer-events-none absolute inset-y-0 left-0 w-4 bg-gradient-to-r from-white dark:from-[#22282b] to-transparent md:hidden" /><div className="pointer-events-none absolute inset-y-0 right-0 w-4 bg-gradient-to-l from-white dark:from-[#22282b] to-transparent md:hidden" /></div></div><button type="button" onClick={handleSubmit} disabled={!isActive || disabled} aria-label="Send message" className={cn('p-2 rounded-full transition-all duration-200 shrink-0', getButtonBg())}><div className="block dark:hidden"><ArrowUpIcon size={24} color={isActive && !disabled ? '#ffffff' : '#9fa5ba'} /></div><div className="hidden dark:block"><ArrowUpIcon size={24} color={isActive && !disabled ? '#000000' : '#62737b'} /></div></button></div>
      </div>
    </div>
  );
