...
const Sidebar = () => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey || e.metaKey) {
        switch (e.key.toLowerCase()) {
          case 'n': e.preventDefault(); handleNewChat(); break
          case 'p': e.preventDefault(); router.push('/pricing'); break
  ...
  if (loading) return <div className="w-80 h-full bg-white dark:bg-[#171b1d]" />

  const isActive = (path: string) => pathname === path
  const authPath = user ? '/profile' : '/signin'
  const isAuthActive = pathname === authPath || pathname === '/signin' || pathname === '/profile'
  const getUserAvatar = () => user?.image || user?.avatar || user?.photoURL || user?.profileImage || user?.picture || '/images/default-avatar.svg'
  ...
  return (
    <div className="sidebar-scroll-container flex flex-col p-0 w-80 h-full pb-3 border-r border-neutral-200 dark:border-neutral-800 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 hover:scrollbar-thumb-neutral-500 dark:hover:scrollbar-thumb-neutral-500 bg-white dark:bg-[#171b1d]">
      ...
      <div className="flex flex-col gap-2 p-4 pr-1">
        <div className="relative"><AnimatePresence>{isActive('/') && <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2, ease: 'easeInOut' }} className="absolute left-0 top-1/2 -translate-y-1/2 z-10"><PointIcon size={16} color="#7FF86C" /></motion.div>}</AnimatePresence><div className="flex items-center justify-between w-full"><SidebarItemNew label="New Chat" href="/" variant="internal" type="desktop" onClick={handleNewChat} className={isActive('/') ? 'pl-6' : ''} /><Command label="Ctrl+N" className="shrink-0 ml-2" /></div></div>

        <div className="relative"><AnimatePresence>{isActive('/chats') && <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2, ease: 'easeInOut' }} className="absolute left-0 top-1/2 -translate-y-1/2 z-10"><PointIcon size={16} color="#7FF86C" /></motion.div>}</AnimatePresence><div className="flex items-center justify-between w-full"><SidebarItemAccordion label="Chats" type="desktop" onToggle={(isOpen) => console.log('Accordion is', isOpen ? 'open' : 'closed')} className={isActive('/chats') ? 'pl-6' : ''}><ChatHistory type="desktop" /></SidebarItemAccordion></div></div>
